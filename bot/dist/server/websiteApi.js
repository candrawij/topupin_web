...
function getRoleLink(role, userId, ticketId) {
    if (role === 'customer') {
        if (!userId || !ticketId) {
...
function resolveWebsiteUser(payload) {
...
export function startWebsiteApi(port = defaultPort) {
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method === 'POST' && requestUrl.pathname === '/api/create-transaction') {
            try {
                ...
                const serialized = JSON.parse(JSON.stringify(transaction, (key, value) => typeof value === 'bigint' ? Number(value) : value));
        ...
        if (req.method === 'GET' && requestUrl.pathname === '/api/transactions') {
            try {
                ...
                const serialized = JSON.parse(JSON.stringify(transactions, (key, value) => typeof value === 'bigint' ? Number(value) : value));
                jsonResponse(res, 200, {
                    success: true,
