...
import { appendMessage, assignTicket, getCustomerTicket, getOrCreateCustomerTicket, getTicketById, getUserSession, listAdminTickets, listOpenTickets, listCsTickets, setUserSession, updateUserSession, ticketListText, ticketSummary, updateTicketStatus, rateTicket, } from '../services/ticketStore.js';
...
function telegramPollingConflict(error) {
...
const customerStartTemplate = (name, ticketId) => [
...
const userToAdminTemplate = (userName, userId, message, ticketId, trxInfo) => [
...
const customerReplyTemplate = (name, replyText, ticketId) => [
    '📩 **BALASAN DARI TIM CS**',
    '',
    '━━━━━━━━━━━━━━━━━━━━━',
    `Halo Kak ${name}! 👋`,
...
const transactionStatusTemplate = () => [
...
const transactionDetailsTemplate = (trx) => [
    '📦 **DETAIL TRANSAKSI**',
    '',
    '━━━━━━━━━━━━━━━━━━━━━',
...
function buildCustomerPromptTemplate() {
    return [
...
async function forwardCustomerMessageToAdmin(userId, userName, ticketId, message, trxInfo) {
...
bot.command('start', async (ctx) => {
    ...
    const ticket = await getOrCreateCustomerTicket(telegramId, customerName, initialMessage, payload.websiteUserId);
...
bot.command('help', async (ctx) => {
    ...
    helpLines.push(`🌐 Website: ${config.websiteBaseUrl}`, '📌 *Ketik pesan langsung untuk chat dengan CS.*');
...
bot.on('callback_query:data', async (ctx) => {
    ...
    if (data === 'cancel_order') {
        updateUserSession(telegramId, { orderState: 'idle', orderData: undefined });
        try {
            await ctx.editMessageText('❌ **Pesanan Dibatalkan**\n\nPesanan Anda telah dibatalkan. Ketik `/menu` untuk memulai ulang.', { parse_mode: 'Markdown', reply_markup: undefined });
        }
        catch (e) {
    ...
    if (data === 'customer_website') {
        ...
        await ctx.reply(`🌐 **Website TopUpIn**\n\nKunjungi website kami untuk melihat katalog produk, melacak pesanan, dan informasi lainnya.`, { parse_mode: 'Markdown', reply_markup: keyboard });
...
bot.on('message:text', async (ctx) => {
    if (config.adminGroupChatId && ctx.chat.id === config.adminGroupChatId) {
        ...
        if (repliedMessage && adminReplyMap.has(repliedMessage.message_id)) {
            ...
            if (forwardReference) {
                const responseText = customerReplyTemplate(forwardReference.userName, ctx.message.text ?? '', forwardReference.ticketId);
    ...
    if (role === 'customer') {
        ...
        await forwardCustomerMessageToAdmin(telegramId, customerName, ticket.ticketId, text, trxInfo);
        if (isFirstMessage) {
    ...
    if (role === 'cs') {
        ...
        if (ticket.userId) {
            const user = await prisma.user.findUnique({ where: { id: ticket.userId } });
            if (user && user.telegramId) {
                const responseText = customerReplyTemplate(user.name, text, ticket.ticketId);
                try {
